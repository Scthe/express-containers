...
function getPackageEntryPoint(dirPath) {
  ...
  try {
    if (existsSync(join(dirPath, 'package.json'))) {
      entryPoint =
        JSON.parse(
          readFileSync(join(dirPath, 'package.json'), { encoding: 'utf8' })
        ).main || entryPoint;
    }
  } catch (ignored) {
    // ignored
...
export function getDynamicRequireModules(patterns, dynamicRequireRoot) {
  ...
  for (const pattern of !patterns || Array.isArray(patterns)
    ? patterns || []
    : [patterns]) {
    ...
    for (const path of new fdir()
      ...
      if (isDirectory(resolvedPath)) {
        ...
        const modulePath = resolve(
          join(resolvedPath, getPackageEntryPoint(path))
        );
  ...
  return {
    commonDir: dirNames.size
      ? getCommonDir([...dirNames, dynamicRequireRoot])
      : null,
...
export function getDynamicModuleRegistry(
  ...
  const dynamicModuleProps = [...dynamicRequireModules.keys()]
    .map(
      (id, index) =>
        `\t\t${JSON.stringify(
          getVirtualPathForDynamicRequirePath(id, commonDir)
        )}: ${
          id.endsWith('.json')
            ? `function () { return json${index}; }`
            : `require${index}`
        }`
    )
    .join(',\n');
